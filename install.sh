...
def get_hardware_fingerprint():
    mac = subprocess.getoutput('cat /sys/class/net/eth0/address 2>/dev/null || cat /sys/class/net/wlan0/address').strip()
    serial = subprocess.getoutput("dmesg | grep -o 'Serial: [^ ]*' | head -1 | cut -d\\  -f2").strip()
    model = subprocess.getoutput("cat /proc/device-tree/model 2>/dev/null").strip()
    return hashlib.sha256(f"{mac}{serial}{model}".encode()).hexdigest()
